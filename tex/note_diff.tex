...
%%%%% Generated with the online version of latexdiff https://github.com/ftilmann/latexdiff/
%%%%% from https://3142.nl/latex-diff/
...
\begin{abstract}
  \noindent Every ten years, the United States Census Bureau collects data on all people living in the US, including information on age, sex, race, ethnicity, and household \DIFdelbegin \DIFdel{relationship}\DIFdelend \DIFaddbegin \DIFadd{relationships}\DIFaddend .
  The Census Bureau is required by law to protect \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend data from disclosure where data provided by any individual can be identified, and, \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{for the first two major releases from the }\DIFaddend 2020 \DIFaddbegin \DIFadd{Decennial Census}\DIFaddend , it used a novel approach to meet this requirement, the \DIFdelbegin \DIFdel{differentially private }\DIFdelend TopDown Algorithm.
  ...
  Our simulation assumed that in Texas, 0.2\% of the 3,\DIFdelbegin \DIFdel{095,857 }\DIFdelend \DIFaddbegin \DIFadd{009,117 }\DIFaddend residents who were under the age of 8 in the 2010 census were transgender and would have a different sex reported in the 2020 census, and 23\% would reside at the same address, which implied that 1,\DIFdelbegin \DIFdel{424 }\DIFdelend \DIFaddbegin \DIFadd{384 }\DIFaddend trans youth were at risk of having their transgender identity status disclosed by a reconstruction-abetted linkage attack. We found that without any disclosure avoidance in 2010 or 2020, a reconstruction-abetted linkage attack identified \DIFdelbegin \DIFdel{657 }\DIFdelend \DIFaddbegin \DIFadd{691 }\DIFaddend transgender youth.  With 5\% swapping in 2010 and 2020, it identified \DIFdelbegin \DIFdel{605 individuals, an 8}\DIFdelend \DIFaddbegin \DIFadd{657 individuals, a 5}\DIFaddend \% decrease.  With swapping in 2010 and TopDown in 2020 as configured for the actual data release, it identified 194 individuals, a \DIFdelbegin \DIFdel{68}\DIFdelend \DIFaddbegin \DIFadd{70}\DIFaddend \% decrease from swapping.  \DIFdelbegin \DIFdel{Our simulation }\DIFdelend \DIFaddbegin \DIFadd{By comparing this to a scenario where sex data was replaced with completely random data, we }\DIFaddend found that the TopDown configuration attains the maximum achievable level of privacy protection against such an attack\DIFaddbegin \DIFadd{, identifying no more individuals than in a scenario where all information on sex was replaced with random noise}\DIFaddend .
...
As part of the 2020 decennial census, the US Census Bureau \DIFdelbegin \DIFdel{has }\DIFdelend developed a new approach to disclosure avoidance, based on differential privacy, called the TopDown Algorithm (TDA) (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{abowd2019census}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Abowd20222020}}\hskip0pt%DIFAUXCMD
...
%DIF < [[refs go here one day]]
and we agree that there are broad, political implications behind statistical accuracy; the framing of census data informs everything from the shape and number of legislative districts to funding and resourcing for minority groups (see, for example, \cite{thompson2012making}). But this is also true of privacy---accurate representation is not an unalloyed good. For many groups, particularly those who are vulnerable to and have experienced active discrimination by state entities, higher accuracy can also mean higher identifiability and higher \textit{scrutiny}. An example of \DIFdelbegin \DIFdel{this is undocumented immigrants' relation to questions about citizenship---questions that can }\DIFdelend \DIFaddbegin \DIFadd{these concerns surfacing can be seen in the debate surrounding asking questions about citizenship in the decennial census. The 2020 decennial census ended up }\emph{\DIFadd{not}} \DIFadd{including such a question, and proposals to include it were ultimately shelved amidst widespread concern from public advocacy groups that the question could }\DIFaddend be used to identify, surveil, and punish \DIFdelbegin \DIFdel{people who are undocumented , and consequently }\DIFdelend \DIFaddbegin \DIFadd{undocumented immigrants, and further }\DIFaddend lead to reduced engagement with and trust of the census (see \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{barreto2019}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{levitt2019citizenship,baum2022sensitive}}\hskip0pt%DIFAUXCMD
}\DIFaddend ). More recent in the public eye (although just as longstanding, as highlighted by \cite{canaday2009straight}) are questions of gender (\cite{singer2015profusion}), on which this investigation is focused.
...
As all of this suggests, there are many reasons for us to be cautious around data availability and the pursuit of accuracy as an exclusive goal of work in this area. While accuracy is (obviously) beneficial from a statistical perspective, an absence of privacy simultaneously risks both producing real, material harms for the individuals identified \DIFdelbegin \DIFdel{, }\DIFdelend and undermining trust in the census itself and so paradoxically reducing the very accuracy that is aimed for. To demonstrate the importance of factoring identifiability into account---and the necessity of an emphasis on disclosure avoidance in census policy---we used simulation to investigate a risk to privacy, by focusing on the risk of disclosing a child's transgender status through discordant reporting of binary gender in successive censuses.

 \section*{Methods}

We used computer simulation to compare the number of trans youth who might be identified in a simulated population under alternative scenarios of disclosure avoidance.  Our approach began with a simulated population of size and structure similar to the state of Texas, derived from a reconstruction of the US population on April 1, 2010.  \DIFaddbegin \DIFadd{To obtain this reconstruction, for each census block, we used integer programming to maximum a random objective function for a fully saturated contingency table of stratified person counts (stratified by age, sex, race, and ethnicity) subject to constraints derived from census tables P8, P9, P11, P12, and P14, which provided person counts for each census block stratified by combinations of age, sex, race, and ethnicity.  Table P14 was particularly valuable for this youth-focused reconstruction, because it provided counts stratified by sex and single-year age group for those under age 20.
}

\DIFaddend Since our focus is on linking youth between the 2010 and 2020 decennial censuses, we included simulants from this population who were aged \DIFdelbegin \DIFdel{zero to 7 }\DIFdelend \DIFaddbegin \DIFadd{10 to 17 }\DIFaddend and therefore would be under 18 on April 1, \DIFdelbegin \DIFdel{2020.  }\DIFdelend \DIFaddbegin \DIFadd{2020, but also be born by April 1, 2010.  }\DIFaddend We augmented this reconstruction by assigning the simulant's gender based on \DIFaddbegin \DIFadd{tabulated statistics summarizing }\DIFaddend responses to the Sexual Orientation and Gender Identity (SOGI) module of the Behavioral Risk Factors Surveillance System (BRFSS) collected in 2019 (\cite{brfss2019}). %DIF <  [[Described in detail in the appendix??]]
\DIFaddbegin \DIFadd{To avoid making the simulation more complicated than necessary, we did not include correlation between gender and any other simulant attribute (such as race, ethnicity, or age).
}\DIFaddend 

We initialized each simulant with attributes for age, gender, race, ethnicity, and household, where age was an integer value representing the age in years, gender was a five-valued variable (with values of transgender boy; transgender girl; other transgender; cisgender boy; and cisgender girl), race was a 63-valued variable encoding the possible combinations of the six census racial categories, ethnicity was a two-valued variable for Hispanic/non-Hispanic, and household was an identifier that encoded census geography (state, county, tract, block) as well as housing unit \DIFdelbegin \DIFdel{id}\DIFdelend \DIFaddbegin \DIFadd{ID}\DIFaddend .

From this \DIFdelbegin \DIFdel{initial }\DIFdelend \DIFaddbegin \DIFadd{simulated 2020 }\DIFaddend population, we \DIFdelbegin \DIFdel{simulated the progression of time and the data captured in the }\DIFdelend \DIFaddbegin \DIFadd{then simulated time going backwards to }\DIFaddend 2010 and \DIFdelbegin \DIFdel{2020 decennial censuses }\DIFdelend \DIFaddbegin \DIFadd{used these populations to generate simulated census data }\DIFaddend as follows: we recorded the age \DIFdelbegin \DIFdel{at initialization }\DIFdelend \DIFaddbegin \DIFadd{in 2020 }\DIFaddend precisely for each simulant's reported age in the \DIFdelbegin \DIFdel{2010 }\DIFdelend \DIFaddbegin \DIFadd{2020 }\DIFaddend census, and then that age \DIFdelbegin \DIFdel{plus }\DIFdelend \DIFaddbegin \DIFadd{minus }\DIFaddend 10 for each simulant's reported age in the \DIFdelbegin \DIFdel{2020 }\DIFdelend \DIFaddbegin \DIFadd{2010 }\DIFaddend census.  We used a simple model of the other key demographic factors of births, deaths, in-migration, and out-migration to simulate how this population might \DIFdelbegin \DIFdel{change over the next }\DIFdelend \DIFaddbegin \DIFadd{have changed over the }\DIFaddend decade: since our interest was in linking between censuses\DIFaddbegin \DIFadd{, }\DIFaddend we focused on migration \DIFdelbegin \DIFdel{, }\DIFdelend and posited that every household might \DIFdelbegin \DIFdel{move}\DIFdelend \DIFaddbegin \DIFadd{have moved}\DIFaddend , making it harder to link. To realize this household mobility, we selected households to stay unmoved from 2010 to 2020 independently, with probability value $p_{\text{stay}} = 23\%$\DIFaddbegin \DIFadd{, which we }\DIFaddend derived from the American Communities Survey (we obtained this value by calculating the sample-weighted proportion of with-children households that had been in residence for at least 10 years in the 2020 five-year Public Use Microdata Sample).  We \DIFdelbegin \DIFdel{updated the 2020 }\DIFdelend \DIFaddbegin \DIFadd{then changed the 2010 }\DIFaddend address of each non-staying household by selecting a new household for them to move \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend uniformly at random from all simulated households in Texas that were occupied on Census Day 2010, which offered a simple way to include realistic heterogeneity in population density.
...
We then identified all simulants who did not \DIFdelbegin \DIFdel{appear to have moved}\DIFdelend \DIFaddbegin \DIFadd{move}\DIFaddend , according to their (possibly swapped) reported location in the 2010 and 2020 censuses, who had a unique combination of age, race, ethnicity, and geography attributes recorded in both censuses, and counted the simulants in this group with differing values reported for sex in 2010 and 2020.  This constituted our estimate of the number of trans youth who would have their gender identity revealed by a reconstruction-abetted linkage attack if the tables used for reconstruction were protected by swapping.  We hypothesized that this total would be 5\DIFaddbegin \DIFadd{\% }\DIFaddend to 10\% lower than the total from the no-disclosure-avoidance scenario and therefore would also reveal sensitive information about hundreds of trans youth.

\emph{TDA for disclosure avoidance (Scenario 4):} \DIFdelbegin \DIFdel{We were not able to approach this scenarioin a way completely analogous to Scenarios 1-3.  Instead of running TDA ourselves on our simulated data after simulating forward 10 years}\DIFdelend \DIFaddbegin \DIFadd{For this scenario}\DIFaddend , we used the Census Bureau's DHC demonstration product released in April 2023 to generate our estimate of the risk of a reconstruction-abetted linkage attack in this scenario\DIFdelbegin \DIFdel{, which requires additional explanation compared to the previous three scenarios.  }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.  }\DIFaddend The final DHC demonstration product included a Privacy-Protected Microdata File (PPMF) for the 2010 decennial census, consisting of a row for each individual and columns for the attributes of age, sex, race, ethnicity, and geography, and we used this for our 2020 simulated data \DIFdelbegin \DIFdel{. We then generated an ReMF from }\DIFdelend \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\cite{census2023demonstration}}\hskip0pt%DIFAUXCMD
). We used the same reconstructed microdata file described above, which was based on the }\DIFaddend SF1 tables published as part of the 2010 decennial census (\DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{and therefore }\DIFaddend used swapping to protect against disclosure), and \DIFdelbegin \DIFdel{instead of initializing our simulated population in 2010 and simulating the progression of time forward to 2020, we }\DIFdelend initialized our simulated population in 2020, based on the individuals aged 10 to 17 in \DIFdelbegin \DIFdel{the SF1 ReMF}\DIFdelend \DIFaddbegin \DIFadd{this reconstructed microdata}\DIFaddend .  We then simulated the \DIFdelbegin \emph{\DIFdel{regression}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{regression }\DIFaddend of time, going backward from 2020 to the 2010 Census Day, when each simulant would be 10 years younger.  We applied our migration model to keep the location in 2010 identical to that in 2020 for only a random fraction of simulants, governed again by the parameter $p_{\text{stay}}$.

As in the other scenarios, we endowed each simulant with a gender attribute, which we calibrated to match to measurements from the 2019 BRFSS SOGI module. \DIFdelbegin \DIFdel{However, in this scenario, we first set the reported sex in 2020 to match that in the SF1 ReMF, and then set the gender attribute and reported sex in 2010 conditional on the reported sex in 2020.  }\DIFdelend This allowed us to use the demonstration DHC as our proxy for the privacy afforded by TDA in 2020 in our assessment of the number of trans youth who would have their gender identity revealed by a reconstruction-abetted linkage attack using data protected by swapping in 2010 and TDA in 2020.
...
\DIFaddbegin \DIFadd{To establish a theoretical minimum on the number of simulants who might be identified by such an attack, we also considered a case where all information on sex was replaced with random noise.
}

\DIFaddend \section*{Results}

Our simulated population included 25,145,561 individual simulants, matching exactly the 2010 population count for Texas.  We focused on the simulants aged \DIFdelbegin \DIFdel{zero to 7 on April 1, 2010, }\DIFdelend \DIFaddbegin \DIFadd{8 to 17, }\DIFaddend of which we had 3,\DIFdelbegin \DIFdel{095,857.
%DIF < %% Could add something about average household size or average number and age of children in households by race/ethnicity.
}\DIFdelend \DIFaddbegin \DIFadd{009,117, and used this to initialize our Census Day population in 2020.
}\DIFaddend Among these simulants, 0.53\% were trans, with 0.18\% trans boys, 0.23\% trans girls, and 0.12\% other transgender (closely matching the BRFSS values), which led to \DIFdelbegin \DIFdel{0.2}\DIFdelend \DIFaddbegin \DIFadd{0.20}\DIFaddend \% of the simulants having a different value reported for sex in the 2010 and 2020 censuses.
...
Taking these together implied that in our simulation there were \DIFdelbegin \DIFdel{$(\text{3,095,857} \text{ youth}) \times (0.2\%) \times (23\%) = \text{1,424}$ }\DIFdelend \DIFaddbegin \DIFadd{$(\text{3,009,117} \text{ youth}) \times (0.2\%) \times (23\%) = \text{1,384}$ }\DIFaddend trans youth who were at risk of having their gender identity disclosed by \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend reconstruction-abetted linkage attack.
...
In our scenario where tables like those in SF1 or DHC were published precisely as enumerated, without any disclosure avoidance measures applied (Scenario 2), we found that migration and non-uniqueness substantially reduced the number of trans youth whose gender identity was revealed.  However, there were still 1,\DIFdelbegin \DIFdel{414,929 }\DIFdelend \DIFaddbegin \DIFadd{722,991 }\DIFaddend individuals who were uniquely identified by the age, race, ethnicity, and location in 2010 and 1,\DIFdelbegin \DIFdel{766,968 }\DIFdelend \DIFaddbegin \DIFadd{413,801 }\DIFaddend uniquely identified in 2020.  In our simulation, a reconstruction-abetted linkage attack in this scenario identified \DIFdelbegin \DIFdel{657 }\DIFdelend \DIFaddbegin \DIFadd{691 }\DIFaddend trans youth.

In our next scenario (Scenario 3), we added swapping-based disclosure avoidance to the tables in Scenario 2 and found that with respect to a reconstructed-abetted linkage attack, swapping acted similarly to a small boost in migration to prevent identifying trans youth.  At the 5\% swapping level we used in Scenario 3, we found that a reconstruction-abetted linkage attack identified \DIFdelbegin \DIFdel{605 }\DIFdelend \DIFaddbegin \DIFadd{657 }\DIFaddend trans youth, \DIFdelbegin \DIFdel{an 8}\DIFdelend \DIFaddbegin \DIFadd{a 5}\DIFaddend \% reduction from the number identified in Scenario 2.

Our final scenario was designed to represent to the approach taken by the Census Bureau in their 2020 data release. In this scenario, we considered protecting the tables released from the 2010 census with swapping and the tables from the 2020 census with TDA (Scenario 4).  We found that this afforded substantially more protection than the other scenarios we considered.  \DIFdelbegin \DIFdel{Because of the alternative route we took to constructing this scenario, we used a different initial population, starting with 3,009,117 simulants ages 10 to 17 on April 1, 2020.  }\DIFdelend We found that TDA was successful in preventing the bulk of the identifications from Scenario 3; in our simulation, a reconstruction-abetted linkage attack identified 194 trans youth when TDA was used for disclosure avoidance on the 2020 tables, a \DIFdelbegin \DIFdel{68}\DIFdelend \DIFaddbegin \DIFadd{70}\DIFaddend \% reduction in the number identified when swapping was used in Scenario 3.  This is actually slightly lower than the number identified purely by chance; when we replaced the reported sex columns in the PPMF by a value chosen uniformly at random, the attack identified 214 trans youth\DIFdelbegin \DIFdel{)}\DIFdelend .


\section{Discussion}

Our simulation results demonstrate the magnitude of the threat that a linkage attack designed to identify trans youth might pose.  Were the Census Bureau to publish microdata on the 2010 and 2020 censuses (Scenario 1), it would likely identify the transgender status of over 6,000 trans youth in Texas.  In the approach underlying the most recent demonstration data, on the other hand, a reconstruction-abetted linkage attack would likely identify the transgender status of only 194 trans youth in Texas.  Since this is less than the number of trans youth identified when we replaced reported sex with a uniformly random value, it shows that TDA as configured for the 2020 DHC attains the maximum achievable level of privacy protection against this reconstruction-abetted linkage attack\DIFaddbegin \DIFadd{, identifying no more individuals than in a scenario where information on sex was replaced entirely by random noise}\DIFaddend .

This is yet another demonstration of the need for disclosure avoidance in the decennial census.  The bulk of previously published investigations into the quality of TDA demonstration products have compared with published results from the 2010 census \DIFdelbegin \DIFdel{, }\DIFdelend and often reported differences.  But in such comparisons, there is an important limitation because they compare the (published) results of swapping to the (demonstration) results of TDA applied to the unswapped data.  Thus, the conclusion of such a comparison is typically limited to proving that the noise introduced by TDA is different from the noise from swapping.
