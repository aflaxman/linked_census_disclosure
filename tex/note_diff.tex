...
\begin{abstract}
  ...
  We conducted a simulation study to investigate the risk of disclosing a change in how an individual's sex was recorded in successive censuses.  In a simulated population based on a reconstruction of the 2010 decennial census of Texas, we compared the number of transgender individuals under 18 identified by linking \DIFdelbegin \DIFdel{simulated }\DIFdelend census data from \DIFaddbegin \DIFadd{a simulated }\DIFaddend 2010 \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{census and a simulated }\DIFaddend 2020 \DIFaddbegin \DIFadd{census }\DIFaddend under alternative approaches to disclosure avoidance, including swapping in 2020 (as used in 2010) and TopDown in 2020 (as used for the actual data released from the 2020 enumeration).

  Our simulation assumed that in Texas, 0.2\% of the 3,009,117 residents who were under the age of 8 in the 2010 census were transgender and would have a different sex reported in the 2020 census, and 23\% would reside at the same address, which implied that 1,384 trans youth were at risk of having their transgender identity status disclosed by a reconstruction-abetted linkage attack. We found that without any disclosure avoidance in 2010 or 2020, a reconstruction-abetted linkage attack identified \DIFdelbegin \DIFdel{691 }\DIFdelend \DIFaddbegin \DIFadd{307 }\DIFaddend transgender youth.  With 5\% swapping in 2010 and 2020, it identified \DIFdelbegin \DIFdel{657 individuals, a 5}\DIFdelend \DIFaddbegin \DIFadd{283 individuals, an 8}\DIFaddend \% decrease.  With swapping in 2010 and TopDown in 2020 as configured for the actual data release, it identified 194 individuals, a \DIFdelbegin \DIFdel{70}\DIFdelend \DIFaddbegin \DIFadd{30}\DIFaddend \% decrease from swapping.  By comparing this to a scenario where sex data was replaced with completely random data, we found that the TopDown configuration attains \DIFdelbegin \DIFdel{the maximum  achievable }\DIFdelend \DIFaddbegin \DIFadd{a baseline maximum  }\DIFaddend level of privacy protection against such an attack, identifying no more individuals than in a scenario where all information on sex was replaced with random noise.
...
We used computer simulation to compare the number of trans youth who might be identified in a simulated population under alternative scenarios of disclosure avoidance.  Our approach began with a simulated population of size and structure similar to the state of Texas, derived from a reconstruction of the US population on April 1, 2010.  To obtain this reconstruction, for each census block, we used integer programming to \DIFdelbegin \DIFdel{maximum }\DIFdelend \DIFaddbegin \DIFadd{maximize }\DIFaddend a random objective function for a fully saturated contingency table of stratified person counts (stratified by age, sex, race, and ethnicity) subject to constraints derived from census tables P8, P9, P11, \DIFdelbegin \DIFdel{P12}\DIFdelend \DIFaddbegin \DIFadd{P12A-N}\DIFaddend , and P14, which provided person counts for each census block stratified by combinations of age, sex, race, and ethnicity.  Table P14 was particularly valuable for this youth-focused reconstruction, because it provided counts stratified by sex and single-year age group for those under age 20.

Since our focus is on linking youth between \DIFaddbegin \DIFadd{simulated versions of }\DIFaddend the 2010 and 2020 decennial censuses, we \DIFdelbegin \DIFdel{included simulants from this population }\DIFdelend \DIFaddbegin \DIFadd{constructed our simulated 2020 population by including simulants from our reconstruction }\DIFaddend who were aged 10 to \DIFdelbegin \DIFdel{17 and therefore would be }\DIFdelend \DIFaddbegin \DIFadd{17.  By construction, these simulants were }\DIFaddend under 18 on April 1, 2020, \DIFdelbegin \DIFdel{but }\DIFdelend \DIFaddbegin \DIFadd{and were }\DIFaddend also be born \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{before }\DIFaddend April 1, \DIFdelbegin \DIFdel{2010.  }\DIFdelend \DIFaddbegin \DIFadd{2010, and therefore might potentially be linked between the two censuses.  }\DIFaddend We augmented this \DIFdelbegin \DIFdel{reconstruction by assigning the }\DIFdelend \DIFaddbegin \DIFadd{simulated population by assigning each }\DIFaddend simulant's gender based on tabulated statistics summarizing responses to the Sexual Orientation and Gender Identity (SOGI) module of the Behavioral Risk Factors Surveillance System (BRFSS) collected in 2019 (\cite{brfss2019}). To avoid making the simulation more complicated than necessary, we did not include correlation between gender and any other simulant attribute (such as race, ethnicity, or age).
...
We then identified all simulants who did not move, according to their (possibly swapped) reported location in the 2010 and 2020 censuses, who had a unique combination of age, race, ethnicity, and geography attributes recorded in both censuses, and \DIFdelbegin \DIFdel{counted }\DIFdelend \DIFaddbegin \DIFadd{identified }\DIFaddend the simulants in this group with differing values reported for sex in 2010 and 2020.  \DIFaddbegin \DIFadd{We then tabulated and reconstructed the individuals in each block with such as simulant, as described in Scenario 2, and counted the simulants who remained population uniques after introducing this aggregation error.  }\DIFaddend This constituted our estimate of the number of trans youth who would have their gender identity revealed by a reconstruction-abetted linkage attack if the tables used for reconstruction were protected by swapping.  We hypothesized that this total would be 5\% to 10\% lower than the total from the no-disclosure-avoidance scenario and therefore would also reveal sensitive information about hundreds of trans youth.
...
In our scenario where tables like those in SF1 or DHC were published precisely as enumerated, without any disclosure avoidance measures applied (Scenario 2), we found that migration and non-uniqueness substantially reduced the number of trans youth whose gender identity was revealed.  However, there were still 1,722,991 individuals who were uniquely identified by the age, race, ethnicity, and location in 2010 and 1,413,801 uniquely identified in 2020.  In our simulation, \DIFaddbegin \DIFadd{we found that if }\DIFaddend a reconstruction-abetted linkage attack in this scenario \DIFaddbegin \DIFadd{was able to attain perfect reconstructed it would }\DIFaddend identified 691 trans youth.  \DIFaddbegin \DIFadd{After introducing aggregation error through tabulation and reconstruction, we found that the attack would still identify 307 trans youth.
}\DIFaddend 

In our next scenario (Scenario 3), we added swapping-based disclosure avoidance to the tables in Scenario 2 and found that with respect to a reconstructed-abetted linkage attack, swapping acted similarly to a small boost in migration to prevent identifying trans youth.  At the 5\% swapping level we used in Scenario 3, we found that a reconstruction-abetted linkage attack \DIFaddbegin \DIFadd{with perfect reconstruction }\DIFaddend identified 657 trans youth\DIFdelbegin \DIFdel{, a 5}\DIFdelend \DIFaddbegin \DIFadd{.  After introducing aggregation error, we found that the attack would still identify 283 trans youth, an 8}\DIFaddend \% reduction from the number identified in Scenario 2.

Our final scenario was designed to represent to the approach taken by the Census Bureau in their 2020 data release. In this scenario, we considered protecting the tables released from the 2010 census with swapping and the tables from the 2020 census with TDA (Scenario 4).  We found that this afforded substantially more protection than the other scenarios we considered.  We found that TDA was successful in preventing the bulk of the identifications from Scenario 3; in our simulation, a reconstruction-abetted linkage attack identified 194 trans youth when TDA was used for disclosure avoidance on the 2020 tables, a \DIFdelbegin \DIFdel{70}\DIFdelend \DIFaddbegin \DIFadd{30}\DIFaddend \% reduction in the number identified when swapping was used in Scenario 3.  This is actually slightly lower than the number identified purely by chance; when we replaced the reported sex columns in the PPMF by a value chosen uniformly at random, the attack identified 214 trans youth.
